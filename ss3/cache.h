...
enum cache_policy {
  ...
  CUSTOM,	/* CUSTOMIZATION allows a custom replacement policy */
  FIFO		/* replace the oldest block in the set */
};

/* CUSTOMIZATION: data for custom replacement policy */
#ifndef CUSTOM_REPL_DATA_TYPE
#define CUSTOM_REPL_DATA_TYPE
typedef unsigned int custom_repl_data_t;
#endif
...
#define WRITE_REFERENCES_TO_FILE 0		/* CUSTOMIZATION: 0 if not writing to file */
...
struct cache_blk_t
{
  ...
  custom_repl_data_t custom_repl_data; /* CUSTOMIZATION: data for custom replacement policy */
  int way_no;                          /* CUSTOMIZATION: always track way no */
...
struct cache_t
{
  ...
  FILE* reference_dump;         /* CUSTOMIZATION: file to dump references to */
  int write_to_file;            /* CUSTOMIZATION: boolean to determine if writing to file */
...
/* CUSTOMIZATION: save cache access record to file */
void record_cache_access(struct cache_t *cp,
                         struct cache_blk_t *blk,
		         enum mem_cmd cmd,
		         md_addr_t addr,
		         md_addr_t tag,
