	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  struct cache_blk_t *blk;
  int i, j, bindex;
  char dump_file_name[25]; /* CUSTOMIZATION */
  ...
  /* CUSTOMIZATION: open file for output */
  cp->write_to_file = 0;
  if (WRITE_REFERENCES_TO_FILE) {
    strncpy(dump_file_name, name, 10);
    strncat(dump_file_name, ".dump.txt", 10);
    cp->reference_dump = fopen(dump_file_name, "w");
    if (cp->reference_dump == NULL) {
  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  /* CUSTOMIZATION: default data for custom replacement policy  */
	  //blk->custom_repl_data = 0;
	  init_custom_repl_data(blk, j);
	  /* CUSTOMIZATION: always track way number */
	  blk->way_no = j;
	}
    }
  return cp;
...
cache_char2policy(char c)		/* replacement policy as a char */
{
  ...
  case 'c': return CUSTOM;             /* CUSTOMIZATION: for custom replacement policy */
  case 'f': return FIFO;
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  fprintf(stream,
	  ...
	  : cp->policy == FIFO ? "FIFO"
	  : cp->policy == CUSTOM ? "Custom" // CUSTOMIZATION
	  : (abort(), ""));
}
...
/* CUSTOMIZATION: record cache access (save stats for each access to file */
void
record_cache_access(struct cache_t *cp,
                    struct cache_blk_t *blk,
		    enum mem_cmd cmd,
		    md_addr_t addr,
		    md_addr_t tag,
  ...
  if (cp->write_to_file) {
    fprintf(cp->reference_dump, "%x %x %x %x %x\n",
            cmd,
	    addr,
	    tag,
	    set,
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case FIFO:
    ...
    break;
  /* CUSTOMIZATION: added custom replacement policy */
  case CUSTOM:
    repl = CACHE_BINDEX(cp, cp->sets[set].blks, get_custom_repl_index(cp, set));
    action = update_custom_repl_data(cp, set);
    //fprintf (stderr, "updating way %0d\n", repl->way_no);
    // TODO: insert code to move to head or tail if requested above
    break;
  case Random:
    {
  ...
  //record_cache_access(cp, repl, cmd, addr, tag, set); /* CUSTOMIZATION used to get trace file */
 ...
 cache_hit: /* slow hit handler */
  ...
  record_cache_access(cp, blk, cmd, addr, tag, set); /* CUSTOMIZATION */
  ...
  if (cp->policy == CUSTOM)
    {
      ...
      action = custom_repl_data_access(cp, set, blk);
      if (action == HEAD) {
	 update_way_list(&cp->sets[set], blk, Head);
      }
    }
